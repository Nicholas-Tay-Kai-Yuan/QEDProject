...
const GroupAssignmentBox = ({ post, assignmentStatus, groupId }) => {
  ...
  return (
    <TouchableOpacity style={styles.view} onPress={post.member_assignment == undefined ? !post.completed_quiz ? () => console.log("not completed"): () => navigate("/viewpastquiz", {state: {quizId: post.completed_quiz}}) : () => {tableVisible ? setTableVisible(false): setTableVisible(true)}}>
