...
import { View, StyleSheet, Text, SafeAreaView, ScrollView, Dimensions, TouchableOpacity, Image, TextInput } from "react-native";
    ...
    generateQuestion: (quizData) => {
        ...
        for (let i = 0; i < numOfQ; i++) {
            ...
            if (quizData.skill_code == 'FRAC_ADD_SUB') {
                ...
                answer = math.fraction(eval(equation.replace("--", "- -")));
                sorted = [];
        ...
        for (let i = 0; i < numOfQ; i++) {
            ...
            if (quizData.skill_code == "FRAC_ADD_SUB") {
                ...
                if (inputA == numerator && inputB == denominator) {
                    ...
                    // review = `<i class="fas fa-times" style="color: #FF0505"></i>  Ans: <sup>${numerator}</sup>&frasl;<sub>${denominator}</sub>`;

                }
            }
            else {
                let input = ('ans' in questionArray[i]) ? answerArray[i].ans : undefined;
                let inputA = ('ansA' in questionArray[i]) ? answerArray[i].ansA : undefined;
                let inputB = ('ansB' in questionArray[i]) ? answerArray[i].ansB : undefined;
